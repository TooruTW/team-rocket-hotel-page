...
const liClass =
  "text-theme-neutral-0 text-base h-14 flex items-center justify-center w-full";
const liClassMd =
  "w-full px-5 text-theme-neutral-0 text-base h-14 flex items-center justify-center w-full";
const aTagClass = "w-fit";
const aTagClassMd = "w-full px-5";
const ulClass = "flex gap-4 w-auto ";
const ulPosition = ref("translate-x-full");
const ulClassMd =
  " absolute transition ease-in-out w-full h-screen left-0 top-0 flex-col items-center gap-4 justify-center bg-theme-neutral-bg ";
...
<template>
  <nav
    class="w-full flex items-center justify-between py-6 max-md:py-4 overflow-x-hidden"
    :class="[classFromParent]"
  >
    <RouterLink class="h-full" to="/">
      <img
        class="h-full max-w-50 max-md:w-27"
        src="/hotel-icon.png"
        alt="hotel-icon"
      />
    </RouterLink>
    ...
    <ul class="flex gap-8 w-auto" :class="[ulClassState, ulPosition]">
      ...
      <RouterLink
        v-show="isLogined"
        :class="aTagClassState"
        to="/userinfo-and-order/myorder"
      >
        <li class="flex gap-2" :class="liClassState">
          ...
          >
            ...
            />
            <path
              d="M4.27344 18.346C4.27344 18.346 6.50244 15.5 12.0024 15.5C17.5024 15.5 19.7324 18.346 19.7324 18.346M12.0024 12C12.7981 12 13.5611 11.6839 14.1238 11.1213C14.6864 10.5587 15.0024 9.79565 15.0024 9C15.0024 8.20435 14.6864 7.44129 14.1238 6.87868C13.5611 6.31607 12.7981 6 12.0024 6C11.2068 6 10.4437 6.31607 9.88112 6.87868C9.31851 7.44129 9.00244 8.20435 9.00244 9C9.00244 9.79565 9.31851 10.5587 9.88112 11.1213C10.4437 11.6839 11.2068 12 12.0024 12V12Z"
              stroke="white"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
      ...
      <RouterLink class="w-32" :class="aTagClassState" to="/rooms">
        <li :class="liClassState" class="bg-theme-primary-100 px-8 rounded-md">
          立即訂房
        </li>
      </RouterLink>
    </ul>
