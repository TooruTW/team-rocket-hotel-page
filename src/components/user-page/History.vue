    ...
    <div class="h-fit overflow-y-hidden" :class="historyClass">
      ...
      >
        ...
        <div class="flex flex-col gap-4">
          ...
          <div>
            <p class="mb-2">住宿天數： {{ countNights(room.checkInDate,room.checkOutDate) }} 晚</p>
            <p>住宿人數：{{ room.peopleNum }} 位</p>
          </div>
          <div>
            <div class="flex items-center gap-3 mb-2">
              <div class="h-6 w-1 rounded-sm bg-theme-primary-100"></div>
              <p>入住：{{ formatDate(room.checkInDate) }}</p>
            </div>
            <div class="flex items-center gap-3">
              <div class="h-6 w-1 rounded-sm bg-theme-neutral-60"></div>
              <p>退房：{{ formatDate(room.checkOutDate) }}</p>
            </div>
          </div>

          <p class="font-bold">NT$ {{ countNights(room.checkInDate,room.checkOutDate) * room.roomId.price }}</p>
        </div>
      </div>
    </div>
    <!-- extend btn -->
