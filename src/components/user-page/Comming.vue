...
<template>
  <div
    class="text-theme-neutral-80 p-10 flex flex-col gap-10 max-lg:p-4 max-lg:gap-6"
  >
    <!-- title -->
    <div>
      <p
    ...
    <!-- img -->
    <img
      class="rounded-md aspect-65/24 w-full"
      :src="roomInfo.roomId.imageUrl"
      alt="房間照片"
    />
    <!-- date -->
    <div class="font-bold max-lg:text-14">
      <h6 class="text-20 leading-[1.2] tracking-wider mb-6 max-lg:text-14">
        {{ roomInfo.roomId.name }}，{{
          countNights(roomInfo.checkInDate, roomInfo.checkOutDate)
        }}
        晚 <span class="mx-2 text-theme-neutral-40">|</span> 住宿人數：{{
          roomInfo.peopleNum
        }}
        位
      </h6>
      <div class="flex items-center gap-3 mb-2">
        ...
        <p>入住：{{ formatDate(roomInfo.checkInDate) }}，15:00 可入住</p>
      </div>
      <div class="flex items-center gap-3 mb-6">
        <div class="h-6 w-1 rounded-sm bg-theme-neutral-60"></div>
        <p>退房：{{ formatDate(roomInfo.checkOutDate) }}，12:00 前退房</p>
      </div>
      <p>
        NT$
        {{
          roomInfo.roomId.price *
          countNights(roomInfo.checkInDate, roomInfo.checkOutDate)
        }}
      </p>
    </div>
    ...
    <div class="flex gap-4">
      <div
        ...
        class="w-full flex justify-center items-center py-4 border-1 rounded-md border-theme-primary-100 text-theme-primary-100"
      >
        取消預訂
      </div>
      <RouterLink
        :to="{
          name: 'RoomDetail',
          params: {
            id: roomInfo.roomId._id,
          },
          query: {
            isEditable: false,
            checkInDate: formatDate(roomInfo.checkInDate),
            checkOutDate: formatDate(roomInfo.checkOutDate),
            price:
              roomInfo.roomId.price *
              countNights(roomInfo.checkInDate, roomInfo.checkOutDate),
          },
        }"
        class="w-full flex justify-center items-center py-4 rounded-md bg-theme-primary-100 text-theme-neutral-0"
      >
        查看詳情
      </RouterLink>
    </div>
    ...
    <div
      v-show="showConfirm"
      class="fixed top-0 left-0 w-full h-full flex justify-center items-center max-md:items-end backdrop-blur-xl max-md:backdrop-blur-none bg-theme-neutral-100/40"
    >
      <div
        class="rounded-md max-md:rounded-b-none bg-theme-neutral-0 flex flex-col items-center justify-between w-150 max-md:w-full h-75 border border-red-100"
      >
        <!-- close btn -->
        <div
          class="flex justify-end max-md:justify-between items-center p-4 w-full max-md:border-b-1 border-theme-neutral-40"
        >
          <h6
            class="font-bold text-14 leading-1.5 tracking-wide hidden max-md:block"
          >
            取消預訂
          </h6>
          <svg
        ...
        <h6
          class="font-bold text-20 max-md:text-14 leading-1.5 tracking-wide text-theme-neutral-80"
        >
          確定要取消此房型的預訂嗎？
        </h6>
        <!-- btns -->
